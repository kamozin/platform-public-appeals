...
Первый экран `/book` оформлен отдельной hero-композицией с обложкой жалобной книги и CTA-ссылками на публичный реестр `/appeals` и форму подачи `/appeal/new`. Общие стили `content-hero` для остальных статических страниц при этом не меняются.
